...
public class Main {
	...
	try {
	    ...
	    webAppContext
		    .setBaseResourceAsPath(Paths.get(Main.class.getClassLoader().getResource("ui/static").toURI()));
	    ...
	    staticHolder.setInitParameter("resourceBase",
		    Main.class.getClassLoader().getResource("ui/static").toExternalForm());
	    ...
	    var deviceRequestHolder = new ServletHolder(new DeviceRequestServlet(lwm2mServer));
	    webAppContext.addServlet(deviceRequestHolder, "/req/*");
	    var eventServlet = new ServletHolder(new DeviceDetailsDataEventServlet(lwm2mServer.getDataStore()));
	    webAppContext.addServlet(eventServlet, "/event");
