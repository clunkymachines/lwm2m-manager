  ...
  public Lwm2mServer(int localPort, int localSecurePort, SecurityStore securityStore) {
    ...
    server.getSendService().addListener(new SendListener() {
		...
		public void dataReceived(Registration registration, TimestampedLwM2mNodes data, SendRequest request) {
			for (Entry<LwM2mPath, LwM2mNode> entry : data.getMostRecentNodes().entrySet()) {
				if (entry.getValue() instanceof LwM2mSingleResource) {
					LwM2mSingleResource resource = (LwM2mSingleResource) entry.getValue();
					dataStore.push(registration.getEndpoint(), entry.getKey().toString(), resource.getValue());	
				}
